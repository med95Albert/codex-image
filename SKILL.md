...
# codex-image ・ 用 Codex CLI 生圖（吃訂閱額度）v1.2.1
...
**QR code 鐵律（v1.2.1 起，事故驅動加入）**：

- **只准標準編碼器**：Python `qrcode`/`segno`、`qrencode` CLI，或 macOS 內建 CoreImage
  `CIQRCodeGenerator`（swift 一行呼叫、零安裝）。**嚴禁手寫 QR 編碼演算法**。
- **驗證要獨立**：用與編碼器不同來源的解碼器驗（macOS Vision framework、zbar、OpenCV）。
  自己編＋自己解＝自洽但可能不合規，不算驗證。
- **裝不了就誠實失敗**，不要硬做。

> 實測事故（2026-07-09）：Codex 沙箱裡沒有 `qrcode` 套件也連不上 PyPI，於是手寫純 Python
> 編碼器並「自我驗證通過」——結果 Apple Vision 完全解不出來，真手機也掃不了。
> 這條鐵律已寫進 `gen_image.sh` 的指令。收到 QR 成品後，Claude 端仍建議獨立複驗一次
> （macOS：CoreImage 生成＋Vision 解碼都內建，零安裝）。
